...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--path",
        default='media/train_data',
        required=True,
        help="Path to male/female training data directories",
        type=str,
    )
    parser.add_argument(
        "--output",
        default=None,
        help="Output file for saving best classifier",
        type=str,
    )

    args = parser.parse_args()
    F_PATH = os.path.join(args.path, 'female')
    M_PATH = os.path.join(args.path, 'male')
    OUTPUT = args.output
